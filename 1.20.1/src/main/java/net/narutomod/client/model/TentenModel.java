...
public final class TentenModel extends EntityModel<TentenEntity> implements ArmedModel {
    public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(
            NarutomodMod.location("tenten_legacy"),
            "main"
    );
    ...
    public TentenModel(ModelPart root) {
    ...
    public static LayerDefinition createBodyLayer() {
        ...
        root.addOrReplaceChild(
                ...
                CubeListBuilder.create()
                        .texOffs(32, 0)
                        .addBox(-4.0F, -8.0F, -3.3F, 8.0F, 8.0F, 8.0F, new CubeDeformation(1.0F)),
                PartPose.ZERO);
        root.addOrReplaceChild(
                "body",
                CubeListBuilder.create()
    ...
    @Override
    public void setupAnim(TentenEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        applyHeldItemPose(entity);
    }

    private void applyHeldItemPose(TentenEntity entity) {
        boolean rightMainArm = entity.getMainArm() == HumanoidArm.RIGHT;
        if (!entity.getMainHandItem().isEmpty()) {
            poseItemArm(rightMainArm ? this.rightArm : this.leftArm);
        }
        if (!entity.getOffhandItem().isEmpty()) {
            poseItemArm(rightMainArm ? this.leftArm : this.rightArm);
        }
        if (entity.isUsingItem()) {
            boolean usingMainHand = entity.getUsedItemHand() == InteractionHand.MAIN_HAND;
            poseItemArm(usingMainHand == rightMainArm ? this.rightArm : this.leftArm);
        }
    }

    private static void poseItemArm(ModelPart arm) {
