...
public final class FourTailsModel extends HumanoidModel<TailedBeastEntity> {
    public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(
            NarutomodMod.location("four_tails"),
            "main"
    );

    private final ModelPart root;
    private final ModelPart jaw;
    private final ModelPart[][] tails = new ModelPart[4][6];
    private final float[][] tailSwayX = new float[4][6];
    private final float[][] tailSwayZ = new float[4][6];

    public FourTailsModel(ModelPart root) {
        super(root);
        this.root = root;
    ...
    public static LayerDefinition createBodyLayer() {
        return LegacyModelLayerDefinitions.EntityFourTails_ModelFourTails_229();
    }
    ...
    public void setupAnim(TailedBeastEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks,
        ...
        this.rightLeg.z += 6.65F;
        this.leftLeg.y += 7.5F;
        this.leftLeg.z += 6.65F;

        for (int i = 0; i < this.tails.length; i++) {
            this.tails[i][0].visible = i < entity.getTailCount();
            for (int j = 1; j < this.tails[i].length; j++) {
                float baseX = j <= 3 ? 0.2618F : -0.2618F;
