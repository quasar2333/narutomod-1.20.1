...
public final class KirinModel extends EntityModel<KirinEntity> {
    public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(
        NarutomodMod.location("kirin_dragon_head"),
        "main"
    );
    ...
    private final ModelPart[] spine = new ModelPart[KirinEntity.SPINE_SEGMENTS];
    private final ModelPart eyes;

    public KirinModel(ModelPart root) {
    ...
    @Override
    public void setupAnim(KirinEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        this.head.xRot = headPitch * Mth.DEG_TO_RAD;
        this.jaw.xRot = entity.tickCount > KirinEntity.WAIT_TICKS ? 0.5236F : 0.0F;
        for (int i = 2; i < 6; i++) {
    ...
    private static void addWhisker(PartDefinition head, String name, float x, float baseYRot, float segmentYRot, boolean mirror) {
        PartDefinition current = head.addOrReplaceChild(
            name + "_0",
    ...
    private static void addSpine(PartDefinition root) {
        PartDefinition current = root;
        for (int i = 0; i < KirinEntity.SPINE_SEGMENTS; i++) {
            current = current.addOrReplaceChild(
                "spine_" + i,
                addBox(
    ...
    private static CubeListBuilder box(int u, int v, float x, float y, float z, float width, float height, float depth, float deformation, boolean mirror) {
        return addBox(CubeListBuilder.create(), u, v, x, y, z, width, height, depth, deformation, mirror);
    }

    private static CubeListBuilder addBox(CubeListBuilder builder, int u, int v, float x, float y, float z, float width, float height, float depth, float deformation, boolean mirror) {
        builder.texOffs(u, v);
        if (mirror) {
            builder.mirror();
        }
