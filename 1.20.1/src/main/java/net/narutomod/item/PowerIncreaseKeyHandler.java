...
public final class PowerIncreaseKeyHandler {
    ...
    public static boolean handle(ServerPlayer player, boolean pressed) {
        ...
        if (head.is(ModItems.MANGEKYOSHARINGANOBITOHELMET.get()) && !isRidingOwnedSusanoo(player)) {
            if (!pressed) {
    ...
    private static boolean isRidingOwnedSusanoo(ServerPlayer player) {
        return player.getVehicle() instanceof AbstractSusanooEntity susanoo && susanoo.isOwnedBy(player);
    }
}
